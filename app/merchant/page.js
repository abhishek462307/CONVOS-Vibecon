...
export default function MerchantDashboard() {
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden" style={{
      ...
      {/* ── TOP HEADER ──────────────────────────────────────── */}
      <header className="h-[52px] shrink-0 flex items-center px-5 gap-3 relative z-30" style={{ background: '#F5F3EF', borderBottom: '1px solid #E8E4DE' }}>

        {/* Logo */}
        <button onClick={() => setActiveSection('home')} className="flex items-center gap-2 shrink-0 hover:opacity-75 transition-opacity">
          <div className="w-6 h-6 rounded-md flex items-center justify-center shrink-0" style={{ background: 'linear-gradient(135deg, #a855f7, #ec4899)' }}>
          ...
          <span className="text-sm font-bold tracking-tight" style={{ color: '#1C0A04' }}>Convos</span>
        </button>

        {/* Divider */}
        <div className="w-px h-4 shrink-0" style={{ background: '#E0D8D0' }} />

        {/* Search — fixed width, not flex expanding */}
        <div className="relative shrink-0" style={{ width: '220px' }}>
          <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 pointer-events-none" style={{ color: '#B5AFA8' }} />
          <input
            value={headerSearch}
            onChange={e => { setHeaderSearch(e.target.value); setSearchOpen(e.target.value.length >= 2) }}
            onFocus={() => headerSearch.length >= 2 && setSearchOpen(true)}
          ...
          {searchOpen && headerSearch.length >= 2 && (
            <div className="absolute top-full left-0 mt-1 bg-white rounded-xl border border-gray-100 overflow-hidden z-50" style={{ width: '300px', boxShadow: '0 8px 24px rgba(0,0,0,0.10)' }}>
              {searchResults.length === 0 ? (
                <p className="px-4 py-3.5 text-xs text-gray-400 text-center">No results for "<span className="text-gray-600">{headerSearch}</span>"</p>
              ) : (
                <div className="py-1">
                  {['order', 'product', 'customer'].map(type => {
                    ...
                    return (
                      <div key={type}>
                        <p className="text-[9px] font-black uppercase tracking-widest text-gray-400 px-3 pt-2 pb-0.5">{LABELS[type]}</p>
                        {group.map((r, i) => (
        ...
        {/* Right actions — all h-8, consistent sizing */}
        <div className="flex items-center gap-2 shrink-0">

          {/* Store pill */}
          <button onClick={() => setActiveSection('store-design')}
            className="flex items-center gap-1.5 h-8 px-3 rounded-lg text-xs font-semibold transition-all whitespace-nowrap"
            style={{ background: '#FFFFFF', border: '1px solid #E8E4DE', color: '#1C0A04' }}
            onMouseEnter={e => e.currentTarget.style.borderColor = '#B8A898'}
            onMouseLeave={e => e.currentTarget.style.borderColor = '#E8E4DE'}>
            {storeConfig?.name || 'Your Store'}
            <span className="flex items-center gap-1 text-[9px] font-bold text-emerald-600 border-l border-gray-200 pl-1.5 ml-0.5">
          ...
          <a href="/store" target="_blank" rel="noopener noreferrer">
            <button className="flex items-center gap-1.5 h-8 px-3 rounded-lg text-xs font-semibold transition-all"
              style={{ background: '#FFFFFF', border: '1px solid #E8E4DE', color: '#7A6F66' }}
              onMouseEnter={e => { e.currentTarget.style.borderColor = '#B8A898'; e.currentTarget.style.color = '#1C0A04' }}
          ...
          <button onClick={handleRefresh} title="Refresh"
            ...
            onMouseEnter={e => { e.currentTarget.style.borderColor = '#B8A898'; e.currentTarget.style.color = '#4A2512' }}
          ...
          <div className="relative">
            <button onClick={() => setAvatarMenuOpen(v => !v)}
            ...
            {avatarMenuOpen && (
              <>
                ...
                <div className="absolute right-0 top-full mt-1.5 w-44 bg-white rounded-xl border border-gray-100 shadow-xl z-50 overflow-hidden" style={{ boxShadow: '0 8px 24px rgba(0,0,0,0.10)' }}>
                  <div className="px-3.5 py-3 border-b border-gray-100">
                    <p className="text-xs font-bold text-gray-900">Merchant</p>
                    <p className="text-[10px] text-gray-400 mt-0.5">merchant@demo.com</p>
                  </div>
                  <div className="py-1">
                    {[
                      { label: 'Store Settings', icon: Paintbrush, section: 'store-design' },
                      { label: 'Dashboard', icon: LayoutDashboard, section: 'home' },
                    ].map(item => (
                      <button key={item.label} onClick={() => { setActiveSection(item.section); setAvatarMenuOpen(false) }}
                        className="w-full flex items-center gap-2.5 px-3.5 py-2 text-xs text-gray-700 hover:bg-gray-50 transition-colors">
                        <item.icon className="w-3.5 h-3.5 text-gray-400" /> {item.label}
                      </button>
                    ))}
